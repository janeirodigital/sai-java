...
class RdfHelperTests {
    ...
    void checkGetStatement() {
        Statement statement = getStatement(readableResource, TESTABLE_NAME);
        assertNotNull(statement);
        assertEquals(READABLE_NAME, statement.getObject().asLiteral().getString());

        Statement missing = getStatement(readableResource, TESTABLE_MISSING);
        assertNull(missing);
    }
    ...
    void checkGetRequiredStatement() throws SaiNotFoundException {
        Statement statement = getRequiredStatement(readableResource, TESTABLE_NAME);
        assertNotNull(statement);
        assertEquals(READABLE_NAME, statement.getObject().asLiteral().getString());

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredStatement(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetObject() {
        RDFNode object = getObject(readableResource, TESTABLE_ID);
        assertNotNull(object);
        assertEquals(6, object.asLiteral().getInt());

        RDFNode missing = getObject(readableResource, TESTABLE_MISSING);
        assertNull(missing);
    }
    ...
    void checkGetRequiredObject() throws SaiNotFoundException {
        RDFNode object = getRequiredObject(readableResource, TESTABLE_ID);
        assertNotNull(object);
        assertEquals(6, object.asLiteral().getInt());

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredObject(readableResource, TESTABLE_MISSING);
        });
    ...
    void checkGetObjects() {
        List<RDFNode> objects = getObjects(readableResource, TESTABLE_HAS_COMMENT);
        assertNotNull(objects);
        assertEquals(3, objects.size());

        List<RDFNode> missing = getObjects(readableResource, TESTABLE_MISSING);
    ...
    void checkGetRequiredObjects() throws SaiNotFoundException {
        List<RDFNode> objects = getRequiredObjects(readableResource, TESTABLE_HAS_COMMENT);
        assertNotNull(objects);
        assertEquals(3, objects.size());

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredObjects(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetUrlObjects() throws SaiException {
        List<URL> objects = getUrlObjects(readableResource, TESTABLE_HAS_TAG);
        assertNotNull(objects);
        assertEquals(3, objects.size());
        assertTrue(objects.containsAll(READABLE_TAGS));

        assertThrows(SaiException.class, () -> {
            getRequiredUrlObjects(readableResource, TESTABLE_HAS_COMMENT);
        });

        List<URL> missing = getUrlObjects(readableResource, TESTABLE_MISSING);
        assertTrue(missing.isEmpty());
    }
    ...
    void checkGetRequiredUrlObjects() throws SaiNotFoundException, SaiException {
        List<URL> objects = getRequiredUrlObjects(readableResource, TESTABLE_HAS_TAG);
        assertNotNull(objects);
        assertEquals(3, objects.size());
        assertTrue(objects.containsAll(READABLE_TAGS));

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredUrlObjects(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetStringObjects() throws SaiException {
        List<String> objects = getStringObjects(readableResource, TESTABLE_HAS_COMMENT);
        assertNotNull(objects);
        assertEquals(3, objects.size());
        assertTrue(objects.containsAll(READABLE_COMMENTS));

        assertThrows(SaiException.class, () -> {
            getRequiredStringObjects(readableResource, TESTABLE_HAS_TAG);
        });

        assertThrows(SaiException.class, () -> {
            getRequiredStringObjects(readableResource, TESTABLE_HAS_MILESTONE);
        });

        assertThrows(SaiException.class, () -> {
            getRequiredStringObjects(readableResource, TESTABLE_CREATED_AT);
        });

        List<String> missing = getStringObjects(readableResource, TESTABLE_MISSING);
        assertTrue(missing.isEmpty());
    }
    ...
    void checkGetRequiredStringObjects() throws SaiNotFoundException, SaiException {
        List<String> objects = getRequiredStringObjects(readableResource, TESTABLE_HAS_COMMENT);
        assertNotNull(objects);
        assertEquals(3, objects.size());
        assertTrue(objects.containsAll(READABLE_COMMENTS));

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredStringObjects(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetUrlObject() throws SaiException {
        URL object = getUrlObject(readableResource, TESTABLE_HAS_MILESTONE);
        assertNotNull(object);
        assertEquals(READABLE_MILESTONE, object);

        assertThrows(SaiException.class, () -> {
            getUrlObject(readableResource, TESTABLE_CREATED_AT);
        });

        URL missing = getUrlObject(readableResource, TESTABLE_MISSING);
        assertNull(missing);
    }
    ...
    void checkGetRequiredUrlObject() throws SaiNotFoundException, SaiException {
        URL object = getRequiredUrlObject(readableResource, TESTABLE_HAS_MILESTONE);
        assertNotNull(object);
        assertEquals(READABLE_MILESTONE, object);

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredUrlObject(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetStringObject() throws SaiException {
        String object = getStringObject(readableResource, TESTABLE_NAME);
        assertNotNull(object);
        assertEquals(READABLE_NAME, object);

        assertThrows(SaiException.class, () -> {
            getStringObject(readableResource, TESTABLE_CREATED_AT);
        });

        assertThrows(SaiException.class, () -> {
            getStringObject(readableResource, TESTABLE_HAS_MILESTONE);
        });

        String missing = getStringObject(readableResource, TESTABLE_MISSING);
        assertNull(missing);
    }
    ...
    void checkGetRequiredStringObject() throws SaiNotFoundException, SaiException {
        String object = getRequiredStringObject(readableResource, TESTABLE_NAME);
        assertNotNull(object);
        assertEquals(READABLE_NAME, object);

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredStringObject(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetIntegerObject() throws SaiException {
        Integer object = getIntegerObject(readableResource, TESTABLE_ID);
        assertNotNull(object);
        assertEquals(READABLE_ID, object);

        assertThrows(SaiException.class, () -> {
            getIntegerObject(readableResource, TESTABLE_CREATED_AT);
        });

        assertThrows(SaiException.class, () -> {
            getIntegerObject(readableResource, TESTABLE_HAS_MILESTONE);
        });

        Integer missing = getIntegerObject(readableResource, TESTABLE_MISSING);
        assertNull(missing);
    }
    ...
    void checkGetRequiredIntegerObject() throws SaiNotFoundException, SaiException {
        Integer object = getRequiredIntegerObject(readableResource, TESTABLE_ID);
        assertNotNull(object);
        assertEquals(READABLE_ID, object);

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredIntegerObject(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetDateTimeObject() throws SaiException {
        OffsetDateTime object = getDateTimeObject(readableResource, TESTABLE_CREATED_AT);
        assertNotNull(object);
        assertEquals(READABLE_CREATED_AT, object);

        assertThrows(SaiException.class, () -> {
            getDateTimeObject(readableResource, TESTABLE_ID);
        });

        assertThrows(SaiException.class, () -> {
            getDateTimeObject(readableResource, TESTABLE_HAS_MILESTONE);
        });

        OffsetDateTime missing = getDateTimeObject(readableResource, TESTABLE_MISSING);
        assertNull(missing);
    }
    ...
    void checkGetRequiredDateTimeObject() throws SaiNotFoundException, SaiException {
        OffsetDateTime object = getRequiredDateTimeObject(readableResource, TESTABLE_CREATED_AT);
        assertNotNull(object);
        assertEquals(READABLE_CREATED_AT, object);

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredDateTimeObject(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetBooleanObject() throws SaiNotFoundException, SaiException {
        boolean object = getBooleanObject(readableResource, TESTABLE_ACTIVE);
        assertEquals(READABLE_ACTIVE, object);

        assertThrows(SaiException.class, () -> {
            getBooleanObject(readableResource, TESTABLE_CREATED_AT);
        });

        assertThrows(SaiException.class, () -> {
            getBooleanObject(readableResource, TESTABLE_HAS_MILESTONE);
        });

        assertThrows(SaiNotFoundException.class, () -> {
            getBooleanObject(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkGetRequiredBooleanObject() throws SaiNotFoundException, SaiException {
        boolean object = getRequiredBooleanObject(readableResource, TESTABLE_ACTIVE);
        assertEquals(READABLE_ACTIVE, object);

        assertThrows(SaiException.class, () -> {
            getRequiredBooleanObject(readableResource, TESTABLE_CREATED_AT);
        });

        assertThrows(SaiException.class, () -> {
            getRequiredBooleanObject(readableResource, TESTABLE_HAS_MILESTONE);
        });

        assertThrows(SaiNotFoundException.class, () -> {
            getRequiredBooleanObject(readableResource, TESTABLE_MISSING);
        });
    }
    ...
    void checkUpdateNodeObject() throws MalformedURLException, SaiException {
        ...
        updateObject(updatableResource, TESTABLE_HAS_MILESTONE, updatableModel.asRDFNode(node));
        assertEquals(url, getUrlObject(updatableResource, TESTABLE_HAS_MILESTONE));
    }
    ...
    void checkUpdateStringObject() throws SaiException {
        ...
        updateObject(updatableResource, TESTABLE_NAME, name);
        assertEquals(name, getStringObject(updatableResource, TESTABLE_NAME));
    }
    ...
    void checkUpdateUrlObject() throws SaiException, MalformedURLException {
        ...
        updateObject(updatableResource, TESTABLE_HAS_MILESTONE, milestone);
        assertEquals(milestone, getUrlObject(updatableResource, TESTABLE_HAS_MILESTONE));
    }
    ...
    void checkUpdateDateTimeObject() throws SaiException {
        ...
        updateObject(updatableResource, TESTABLE_CREATED_AT, dateTime);
        assertEquals(dateTime, getDateTimeObject(updatableResource, TESTABLE_CREATED_AT));
    }
    ...
    void checkUpdateIntegerObject() throws SaiException {
        int id = 777;
        updateObject(updatableResource, TESTABLE_ID, id);
        assertEquals(id, getIntegerObject(updatableResource, TESTABLE_ID));
    }

    @Test
    @DisplayName("Update boolean object by property")
    void checkUpdateBooleanObject() throws SaiNotFoundException, SaiException {
        updateObject(updatableResource, TESTABLE_ACTIVE, false);
        assertFalse(getBooleanObject(updatableResource, TESTABLE_ACTIVE));
    }
    ...
    void checkUpdateUrlObjects() throws MalformedURLException, SaiException {
        ...
        updateUrlObjects(updatableResource, TESTABLE_HAS_MILESTONE, tags);
        assertTrue(CollectionUtils.isEqualCollection(tags, getUrlObjects(updatableResource, TESTABLE_HAS_MILESTONE)));
    }
    ...
    void checkUpdateStringObjects() throws SaiException {
        ...
        updateStringObjects(updatableResource, TESTABLE_HAS_COMMENT, comments);
        assertTrue(CollectionUtils.isEqualCollection(comments, getStringObjects(updatableResource, TESTABLE_HAS_COMMENT)));
    }

    @Test
    @DisplayName("Get URL value from RDF node")
    void checkNodeToUrl() throws SaiException {
        RDFNode object = getObject(readableResource, TESTABLE_HAS_MILESTONE);
        assertEquals(READABLE_MILESTONE, nodeToUrl(object));
        // Fail when URL is not a resource
        RDFNode notResource = getObject(readableResource, TESTABLE_NAME);
