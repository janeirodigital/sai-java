...
import static com.janeirodigital.sai.core.authentication.AuthorizedSessionHelper.getAccessTokenFromRequest;
...
public class AccessTokenRefresher implements Authenticator {
    ...
    private AuthorizedSession getAuthorizedSession(AuthorizedSession session) {
        try { return this.sessionAccessor.get(session); } catch (SaiException ex) {
    ...
    private AuthorizedSession refreshAuthorizedSession(AuthorizedSession session) {
        try { return this.sessionAccessor.refresh(session); } catch (SaiException ex) {
